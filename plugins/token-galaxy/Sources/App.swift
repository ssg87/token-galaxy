...
final class AppController:NSObject,NSApplicationDelegate,NSTouchBarDelegate,NSMenuDelegate,NSWindowDelegate {
    ...
    func setSelection(_ id:String?){selected=id;scene.selected=id;touchScene?.selected=id;updateLabels()}
    ...
    @objc func showOverview(){
        detailPopover?.close();if overview==nil{overview=OverviewController();overview?.onSelect={[weak self] id in self?.recordInteraction();self?.setSelection(id)}};overview?.update(tasks,observed:observed,increments:[:]);overview?.motionPaused=paused || screenSleeping;overview?.show()
    }
